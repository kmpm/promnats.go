...
func handleDiscoveryPaths(nc *nats.Conn, startport int, host string, timeout time.Duration, refresh func(map[string]discovered) error) func(http.ResponseWriter, *http.Request) {
	return func(w http.ResponseWriter, r *http.Request) {
		...
		discoveries, err = discoverPaths(r.Context(), nc, host, startport, timeout)
		httpsd := []HttpEntry{}
...
func discoverPaths(ctx context.Context, nc *nats.Conn, host string, port int, timeout time.Duration) (discoveries map[string]discovered, err error) {
	var msgs []*nats.Msg
	discoveries = make(map[string]discovered)
	msgs, err = req.Do(ctx, nil, "metrics", 0, timeout, nc)
	if err != nil {
		return
	}
	...
	for _, m := range msgs {
		...
		d := discovered{id: pnid, parts: parts, host: host, port: port}
